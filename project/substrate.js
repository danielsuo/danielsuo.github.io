...
class Crack {
  ...
  void move() {
    ...
    if ((cx>=0) && (cx<dim) && (cy>=0) && (cy<dim)) {
      // safe to check
      if ((cgrid[cy*dim+cx]>10000) || (abs(cgrid[cy*dim+cx]-t)<5)) {
        // continue cracking
        cgrid[cy*dim+cx]=int(t);
      } else if (abs(cgrid[cy*dim+cx]-t)>2) {
